...
@budget_router.message(SetIncomeExpenses.AddExpenses,
                       F.text.isdigit(),
                       F.text.cast(int).as_("expanses"),
                       )
async def enter_expanses(message: types.Message, state: FSMContext, expanses: int):
    """
    Принимает от пользователя сумму расходов и завершает ввод данных.
    """
    await state.update_data(expanses=expanses)
